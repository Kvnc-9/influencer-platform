...
def clear_database():
    """TÜM VERİYİ SİLER"""
    try:
        # Username'i garip bir şey olmayan herkesi sil (Yani hepsini)
...
with st.container():
    ...
    # Toplam Kâr/Zarar göstergesi İSTEK ÜZERİNE KALDIRILDI
...
if response.data:
    ...
    # --- Niche Kontrolü (Hata Düzeltici) ---
    if 'Niche' not in df.columns:
        if 'niche' in df.columns: df['Niche'] = df['niche']
        else: df['Niche'] = "Genel"
    
    # Boş stringleri düzelt
